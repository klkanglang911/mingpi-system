...
**性能优化：**
- Logo 图片压缩优化（1024×1024 → 144×144，69KB → 5KB，减少93%）
- 加载时间从 5.8秒 优化至 <0.5秒，提升 10倍以上

### v1.1 (2026-01-01)

**新增功能：**
- 微信收藏引导蒙版（御笺风格卡片设计，仅微信环境显示，按用户ID独立记录）
- 命批开启时间蒙版（支持倒计时、自定义文案）
- 命批内容加载状态提示（"数据加载中..."灰色斜体样式）

**UI优化：**
- Logo 预加载优化（解决内联CSS阻塞图片加载问题）
- 后台管理操作提示改为固定 Toast 样式（屏幕顶部居中，带动画效果）
- "年批月令"更名为"流星赶月"
- 命批开启弹窗文字横向居中显示

**技术改进：**
- 使用 `<link rel="preload">` 预加载关键图片资源
- 收藏引导状态基于用户ID存储，支持多用户独立记录
...
**蒙版功能：**
- 命批开启蒙版：支持设置开启时间和倒计时显示
- 微信收藏引导：仅微信环境首次访问时显示，引导用户添加收藏

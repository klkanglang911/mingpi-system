...
- **页面字体和字号优化**
  - **流星赶月页面**（main.html）
    - 本月命批标题改为系统字体，字号加大一倍（0.65rem → 1.3rem）
    - 本月命批内容文本字号加大一倍（0.9rem → 1.8rem）
  - **岁运分析页面**（fortune.html）
    - 四柱八字标题改为系统字体，字号加大一倍（0.7rem → 1.4rem）
    - 起运年龄标题改为系统字体，字号加大一倍
    - 运程概览标题改为系统字体，字号加大一倍（0.65rem → 1.3rem）
    - 命局分析标题改为系统字体，字号加大一倍（0.7rem → 1.4rem）
    - 农历年名称改为 Noto Serif SC 字体
  - **个人中心页面**（profile.html）
    - 账户设置标题字号加大一倍（0.65rem → 1.3rem）
...
### Improved / 优化
- **全站前端美学升级**
  - 引入 Noto Serif SC（思源宋体）作为正文字体，提升中文排版质感
  - 引入 ZCOOL XiaoWei（站酷小薇体）作为装饰字体（部分页面）
  - 字体渲染优化：启用连字、抗锯齿、亚像素渲染
  - SVG 噪点滤镜添加纸质纹理背景，营造传统书院美学
  - 多层渐变叠加增强视觉层次感

- **字体加载性能优化**
  - 使用国内 CDN（fonts.font.im）替代 Google Fonts
  - 解决中国大陆用户字体加载慢/失败问题
  - 字体加载速度显著提升，稳定可靠
...
### Added / 新增
- **命局分析年份切换功能**
  - 命局分析页面支持农历年份切换（当前年 + 次年）
  - 年份选择器显示干支年名（如：乙巳年、丙午年）
  - 八字信息固定显示，命局分析内容随年份变化

### Changed / 变更
- **命局分析数据结构调整**
  - `analysis` 字段从 `user_profile` 表迁移到 `user_yearly_fortune` 表
  - 命局分析现在按年度存储，支持每年不同的分析内容
  - 后台管理：用户资料标签页移除命局分析，年度运势标签页新增命局分析输入框

### Improved / 优化
- **后台管理提示文案**
  - 用户资料：提示八字为固定资料，命局分析请在年度运势中录入
  - 年度运势：提示包含大运、流年、四季财官和命局分析内容
...
### Improved / 优化
- **农历卡片信息层级优化**
  - 年份从底部移至顶部，形成「年→月→节气→时间」的自然阅读流
  - 符合中国传统「年月」表达习惯（如：乙巳年丑月）
  - 年份作为上下文标签，小字浅色，不喧宾夺主
  - 切换月份时第一眼即可感知年份变化
